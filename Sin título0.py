...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 29 17:01:11 2025
...
num_filas = 5
num_cols = 5
total_imagenes = num_filas * num_cols # Esto será 25

# El índice inicial para las imágenes (ej: empezar desde la fila 10 del CSV)
indice_inicial = 30000

# Asegúrate de que 'archivo2' esté cargado:
# archivo2 = pd.read_csv('nombre_de_tu_segundo_archivo.csv', header=None)
mapa_caracteres = dict(zip(archivo1['class'], archivo1['char']))

# --- PASO 2: Crear la Figura y los Ejes (Subplots) ---

# Creamos una figura con 5 filas y 5 columnas.
# Ajustamos el figsize para que las imágenes se vean más grandes y claras (ej: 10x10)
fig, axes = plt.subplots(
    nrows=num_filas,      # 5 filas
    ncols=num_cols,       # 5 columnas
    figsize=(10, 10)      # Tamaño de la figura (10 pulgadas de ancho, 10 de alto)
)

# Aplanar el array 'axes'. Esto es crucial.
# Cuando tienes más de una fila, 'axes' es un array 2D (5x5). Lo convertimos a 1D (25)
# para que la iteración con un solo índice 'k' sea más fácil.
axes = axes.flatten()

# --- PASO 3: Iterar y Dibujar las 25 Imágenes ---

for k in range(total_imagenes):
    
    # 1. Calcular el índice de la fila en el DataFrame
    fila_actual = indice_inicial + k
    
    # 2. Obtener la etiqueta numérica (Label)
    etiqueta = archivo2.iloc[fila_actual, -1]
    
    # 3. Obtener los píxeles (Omitimos la columna 0)
    datos_pixeles = archivo2.iloc[fila_actual, :-1].values
    
    # 4. Reorganizar a 28x28
    img = datos_pixeles.reshape((28, 28))
    
    # 5. Dibujar la imagen en el k-ésimo subgráfico
    axes[k].imshow(img, cmap='gray')
    
    # 6. Poner el título con la etiqueta y limpiar los ejes
    axes[k].set_title(f"Label: {etiqueta}", fontsize=10)
    axes[k].axis('off') # Desactiva el marco y los números de los ejes (x, y)
